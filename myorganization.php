...
if (isset($_SESSION["user"])) {
    ...
    // Query to fetch logout time from logout2 table
    $sql = "SELECT logout_time FROM logout WHERE id = '$userID'"; // Assuming 'logout2' table and 'id' column
    $result = mysqli_query($conn, $sql);
    $userInfo = mysqli_fetch_assoc($result);

    if ($userInfo) {
        $logoutTime = $userInfo["logout_time"]; // Assuming the column name is 'logout_time'

        // Insert the logout time into the login_info table
        $insertSql = "INSERT INTO login_info (logout_time) VALUES ('$logoutTime')";
...
<header class="d-flex justify-content-between my-4">
    ...
    <div>
        <a href="login_info.php" class="btn btn-primary">User Activity Log</a>
...
$sqlSelect = "SELECT * FROM users ORDER BY $sortField $sortOrder LIMIT $offset, $limit";
    ...
    <tbody id="myTable">
        <?php while($data = mysqli_fetch_array($result)): ?>
            <tr>
                ...
                <td>
                    <a href="myorganizationedit.php?id=<?php echo $data['id']; ?>" class="btn btn-warning">Edit</a>
                    <a href="myorganizationdelete.php?id=<?php echo $data['id']; ?>" class="btn btn-danger">Delete</a>
                </td>
            </tr>
        <?php endwhile; ?>
    </tbody>
